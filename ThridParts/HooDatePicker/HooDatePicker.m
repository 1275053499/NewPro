...
- (void)buildSelectorMonthsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineMonthsTop.backgroundColor = RGB(229,229,229);
    ...
    _lineMonthsBottom.backgroundColor = RGB(229,229,229);
...
- (void)buildSelectorLabelsMonths {
    ...
    CGFloat offsetContentScrollView = (_scrollViewYears.frame.size.height - kHooDatePickerScrollViewItemHeight) / 2.0;
//    NSLog(@"%f",offsetContentScrollView);
...
- (void)buildSelectorYearsOffsetX:(CGFloat)x andWidth:(CGFloat)width {
    ...
    _lineYearsTop.backgroundColor = RGB(229,229,229);
    ...
    _lineYearsBottom.backgroundColor = RGB(229,229,229);
...
-(void)dismiss {
    ...
    [UIView animateWithDuration:kHooDatePickerAnimationDuration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        self.frame = CGRectMake(self.frame.origin.x, _superView.frame.size.height, self.frame.size.width, self.frame.size.height);
//        NSLog(@"%lf",_superView.frame.size.height);
...
//自己修改 超过了数组数量最大值 需要减一个单位
    if (scrollView == _scrollViewMonths) {
        if (index==self.months.count) {
            index--;
        }
    }
    
    if (scrollView == _scrollViewYears) {
        if (index==self.years.count) {
            index--;
        }
    }
...
#pragma mark - Getters and Setters    //修改背景
...
- (UIView *)dimBackgroundView {
    if(!_dimBackgroundView) {
        ...
        _dimBackgroundView.backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.5];  //修改背景
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        [sureButton setTitleColor:RGB(102,102,102) forState:UIControlStateNormal];
